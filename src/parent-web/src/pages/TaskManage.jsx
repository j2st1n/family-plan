...
export default function TaskManage({ token, child, onBack }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}><h2 style={{ fontSize: "1.3rem", fontWeight: 700 }}>{child.name}</h2><div style={{ fontSize: "0.9rem", color: "#73706b" }}>⭐{rewards.stars_total} 🔥{rewards.current_streak_days}</div></div>
  ...
  return (
    <div style={{ ...s.panel, marginTop: "0.8rem" }}>
      ...
      {redemptions.length > 0 && <div style={{ marginTop: "0.8rem", borderTop: "1px solid #e8e4df", paddingTop: "0.6rem" }}>
        <p style={{ fontSize: "0.8rem", color: "#73706b", marginBottom: "0.3rem" }}>已兑换</p>
        {redemptions.map(r => (
          <div key={r.id} style={s.shopRow}>
            <div style={{ flex: 1 }}>
              <p style={{ fontWeight: 600, fontSize: "0.9rem" }}>{r.title}</p>
              <p style={{ fontSize: "0.75rem", color: "#73706b" }}>⭐{r.star_cost} · {r.child_id ? "孩子" : ""}</p>
            </div>
            {r.redemption_status === "pending" ? (
              <button onClick={() => handleFulfill(r.redemption_id)} style={s.shopAct}>兑现</button>
            ) : (
              <span style={{ fontSize: "0.75rem", color: "#4b9c64" }}>已兑现</span>
            )}
          </div>
        ))}
      </div>}
...
const s = {
  ...
  shopAct: { padding: "0.25rem 0.4rem", borderRadius: "6px", fontSize: "0.75rem", fontWeight: 600, color: "#fff", background: "#3d9e6b" },
